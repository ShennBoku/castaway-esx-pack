...
CreateThread(function ()
    for k=1, #bankActions do
        RegisterNUICallback(bankActions[k], function(data, cb)
            data.kygLoc = kyg.util.getMapNameOfZone(GetEntityCoords(cache.ped))
...
local function initalizeBanking()
    CreatePeds()
    SendNUIMessage({ action = 'updateLocale', translations = lib.getLocales(), currency = Config.currency })
end
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    if not kyg.player.isLoaded() then return end
    initalizeBanking()
end)
