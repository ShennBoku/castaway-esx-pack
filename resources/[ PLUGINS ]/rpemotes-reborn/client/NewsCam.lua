...
if Config.NewscamEnabled then
    ...
    local function CleanupNewscam()
        ...
        if prop_newscam then DeleteEntity(prop_newscam) end
    ...
    function UseNewscam()
        ...
        if IsUsingNewscam then
            CreateThread(function()
                ...
                prop_newscam = CreateObject(lib.requestModel('prop_v_cam_01'), x, y, z + 0.2, true, true, true)
                AttachEntityToEntity(prop_newscam, cache.ped, GetPedBoneIndex(cache.ped, 28422), 0.0, 0.03, 0.01, 0.0, 0.0, 0.0, true, true, false, true, 1, true)

                lib.playAnim(cache.ped, 'missfinale_c2mcs_1', 'fin_c2_mcs_1_camman', 5.0, 5.0, -1, 51, 0, false, 0, false)
            ...
            local breaking_news = lib.requestScaleformMovie('breaking_news')
            ...
            while IsUsingNewscam and not IsEntityDead(cache.ped) and not IsPedSittingInAnyVehicle(cache.ped) do
                ...
                DrawScaleformMovieFullscreen(scaleform_news, 255, 255, 255, 255, 0)
                if instructions then
                    DrawScaleformMovieFullscreen(scaleform_instructions, 255, 255, 255, 255, 0)
                end
                Wait(1)
            end
        end
