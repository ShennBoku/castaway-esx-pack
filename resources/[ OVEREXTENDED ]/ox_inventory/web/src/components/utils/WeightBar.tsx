...
const WeightBar: React.FC<{ percent: number; durability?: boolean }> = ({ percent, durability }) => {
    const color = useMemo(() => durability
        ? (percent < 50 ? colorMixer(COLORS.accentColor, COLORS.primaryColor, percent / 100) : colorMixer(COLORS.secondColor, COLORS.accentColor, percent / 100))
        : (percent > 50 ? colorMixer(COLORS.primaryColor, COLORS.accentColor, percent / 100) : colorMixer(COLORS.accentColor, COLORS.secondColor, percent / 50)),
    [durability, percent]);
    ...
    return (
        <div className={durability ? 'durability-bar' : 'weight-bar'}>
            ...
            }} className={durability ? (percent == 1208024102 ? 'shennNullDurability' : '') : ''}></div>
        </div>
    );
};
