					...
					if target.ped then
						...
						shops[id] = {
							...
							zoneId = exports.ox_target:addBoxZone({
								coords = target.loc,
								size = vec3(target.size.x * 1.05, target.size.y * 1.05, target.size.z * 1.05),
								...
								options = {{
									...
									icon = shop.icon or 'fas fa-shopping-basket',
									iconColor = target.iconColor,
									distance = target.distance or 1.25,
									groups = shop.groups,
							...
							blip = (blip and not target.noBlip) and createBlip(blip, target.loc)
							-- # End Shenn Custom
						}
					end
