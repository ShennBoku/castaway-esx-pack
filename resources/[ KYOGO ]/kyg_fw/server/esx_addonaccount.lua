...
local function CreateAddonAccount(name, owner, money)
	...
	function self.save()
		if self.owner == nil then MySQL.update("UPDATE `addon_account_data` SET `money` = ? WHERE `account_name` = ?", { self.money, self.name })
		else MySQL.update("UPDATE `addon_account_data` SET `money` = ? WHERE `account_name` = ? AND `owner` = ?", { self.money, self.name, self.owner }) end
...
AddEventHandler('onResourceStart', function(resourceName)
	...
	local checkJobComplete = false
	local checkJob = MySQL.query.await("SELECT CONCAT('society_', `name`) AS `society_name`, `label` AS `society_label` FROM `jobs` WHERE `whitelisted` = 1", {})
	if checkJob then for i = 1, #checkJob do
		local socData = checkJob[i]
		MySQL.query.await("REPLACE INTO `addon_account` (`name`, `label`, `shared`) VALUES (?, ?, 1)", { socData.society_name, socData.society_label })
		if i == #checkJob then checkJobComplete = true end
	end end
	while not checkJobComplete do Wait(0) end
	...
	for i = 1, #accounts do
		...
		if account.shared == 0 then
			...
			if account.money then SharedAccounts[account.name] = CreateAddonAccount(account.name, nil, account.money)
			else newAccounts[#newAccounts + 1] = { account.name, 0 } end
		end
	end
...
function GetAccount(name, owner)
	for i = 1, #Accounts[name], 1 do if Accounts[name][i].owner == owner then
		return Accounts[name][i]
	end end
end exports('GetAccount', GetAccount)

function GetSharedAccount(name)
	return SharedAccounts[name]
end exports('GetSharedAccount', GetSharedAccount)
...
function AddSharedAccount(society, amount)
	...
	-- addon account:
	local account = MySQL.insert.await('INSERT INTO `addon_account` (`name`, `label`, `shared`) VALUES (?, ?, ?)', { society.name, society.label, 1 })
	if not account then return end

	-- if addon account inserted, insert addon account data:
	local account_data = MySQL.insert.await('INSERT INTO `addon_account_data` (`account_name`, `money`) VALUES (?, ?)', { society.name, (amount or 0) })
...
end exports('AddSharedAccount', AddSharedAccount)
	...
	for i = 1, #addonAccounts, 1 do
		...
		if shared == 0 then
			...
			if #addonAccountData == 0 then
				...
				money = 0
			else money = addonAccountData[1].money end
