...
local Debug = false
local frameName = SetKygFrame('esx_progressbar')
...
local function Progressbar(message, length, Options)
    if CurrentProgress then return false end
    ...
    SendKygFrameMsg(frameName, { type = 'Progressbar', length = length or 3000, message = message or 'ESX-Framework' })
...
local function CancelProgressbar()
    ...
    SendKygFrameMsg(frameName, { type = 'Close' })
    ClearPedTasks(ESX.PlayerData.ped)
    if CurrentProgress.FreezePlayer then FreezeEntityPosition(ESX.PlayerData.ped, false) end
...
if Debug then
    RegisterCommand('esxprogress', function()
        ESX.Progressbar('test', 5000,{
            FreezePlayer = false,
            animation = { type = 'anim', dict = 'mini@prostitutes@sexlow_veh', lib ='low_car_sex_to_prop_p2_player' },
            onFinish = function()
                --Code here
            end
        })
