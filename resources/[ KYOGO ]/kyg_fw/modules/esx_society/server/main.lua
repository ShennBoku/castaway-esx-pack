...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	...
	for i=1, #(xPlayers) do 
		...
		if cfgSociety.EnableESXIdentity and name == GetPlayerName(xPlayer.source) then
	...
	if cfgSociety.EnableESXIdentity then
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, actionType)
	local xPlayer = ESX.GetPlayerFromId(source)
	local isBoss = cfgSociety.BossGrades[xPlayer.job.grade_name]
...
ESX.RegisterServerCallback('esx_society:setJobSalary', function(source, cb, job, grade, salary)
	...
	if xPlayer.job.name == job and cfgSociety.BossGrades[xPlayer.job.grade_name] then
		if salary <= cfgSociety.MaxSalary then
			...
			print(('[^3WARNING^7] Player ^5%s^7 attempted to setJobSalary over the cfgSociety limit for ^5%s^7!'):format(source, job))
			cb()
		end
	else
