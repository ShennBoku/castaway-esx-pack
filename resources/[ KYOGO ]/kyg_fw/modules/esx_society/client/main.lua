	...
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local elements = {
				...
				{icon = "fas fa-wallet", title = TranslateCap('amount_title'), input = true, inputType = "number", inputPlaceholder = TranslateCap('change_salary_placeholder'), inputMin = 1, inputMax = cfgSociety.MaxSalary, name = "gradesalary"},
				{icon = "fas fa-check", title = TranslateCap('confirm'), value = "confirm"}, 
				{icon = "fas fa-arrow-left", title = TranslateCap('return'), value = "return"}
			}
			...
			if element.value == "confirm" then
				...
				if amount == nil then
					...
					OpenManageSalaryMenu(society, options)
				elseif amount > cfgSociety.MaxSalary then
