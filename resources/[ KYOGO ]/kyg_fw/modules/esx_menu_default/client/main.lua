local frameName = SetKygFrame('esx_menu_default', 'ui')
...
local function openMenu(namespace, name, data)
    ...
    SendKygFrameMsg(frameName, { action = 'openMenu', namespace = namespace, name = name, data = data })
end
...
local function closeMenu(namespace, name)
    ...
    SendKygFrameMsg(frameName, { action = 'closeMenu', namespace = namespace, name = name })
end
...
RegisterNUICallback('esx_menu_default:menu_submit', function(data, cb)
...
RegisterNUICallback('esx_menu_default:menu_cancel', function(data, cb)
    local menu = ESX.UI.Menu.GetOpened(MenuType, data._namespace, data._name)
    if not menu then return end
...
RegisterNUICallback('esx_menu_default:menu_change', function(data, cb)
    local menu = ESX.UI.Menu.GetOpened(MenuType, data._namespace, data._name)
    if not menu then return end
    ...
    if menu.change ~= nil then menu.change(data, menu) end
    cb('OK')
end)
...
ESX.RegisterInput('menu_default_enter', 'Submit menu item', 'keyboard', 'RETURN', function()
    if OpenedMenus > 0 and (GetGameTimer() - GUI.Time) > 200 then
        SendKygFrameMsg(frameName, { action = 'controlPressed', control = 'ENTER' })
...
ESX.RegisterInput('menu_default_backspace', 'Close menu', 'keyboard', 'BACK', function()
    if OpenedMenus > 0 then
        SendKygFrameMsg(frameName, { action = 'controlPressed', control = 'BACKSPACE' })
        GUI.Time = GetGameTimer()
    end
end)
...
ESX.RegisterInput('menu_default_top', 'Change menu focus to top item', 'keyboard', 'UP', function()
    if OpenedMenus > 0 then
        SendKygFrameMsg(frameName, { action = 'controlPressed', control = 'TOP' })
        GUI.Time = GetGameTimer()
    end
end)
...
ESX.RegisterInput('menu_default_down', 'Change menu focus to down item', 'keyboard', 'DOWN', function()
    if OpenedMenus > 0 then
        SendKygFrameMsg(frameName, { action = 'controlPressed', control = 'DOWN' })
        GUI.Time = GetGameTimer()
    end
end)
...
ESX.RegisterInput('menu_default_left', 'Change menu slider to left', 'keyboard', 'LEFT', function()
    if OpenedMenus > 0 then
        SendKygFrameMsg(frameName, { action = 'controlPressed', control = 'LEFT' })
        GUI.Time = GetGameTimer()
    end
end)
...
ESX.RegisterInput('menu_default_right', 'Change menu slider to right', 'keyboard', 'RIGHT', function()
    if OpenedMenus > 0 then
        SendKygFrameMsg(frameName, { action = 'controlPressed', control = 'RIGHT' })
        GUI.Time = GetGameTimer()
    end
end)
