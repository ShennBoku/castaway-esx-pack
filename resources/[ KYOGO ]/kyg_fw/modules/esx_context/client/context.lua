local activeMenu
local Debug = false
local frameName = SetKygFrame('esx_context')

-- Global functions
-- [ Post | Open | Closed ]

local function Post(fn, ...)
    SendKygFrameMsg(frameName, { func = fn, args = { ... }})
end

local function Open(position, eles, onSelect, onClose, canClose)
    local canCloseMenu = canClose == nil and true or canClose
    activeMenu = { position = position, eles = eles, canClose = canCloseMenu, onSelect = onSelect, onClose = onClose }
...
local function Closed()
    SetKygFrameFocus(frameName, false, false)
...
exports('PreviewContext', Open)

exports('OpenContext', function(...)
    Open(...)
    SetKygFrameFocus(frameName, true, true)
end)

exports('CloseContext', function()
    if not activeMenu then return end
    Post('Closed')
    Closed()
end)

exports('RefreshContext', function(eles, position)
    if not activeMenu then return end
...
RegisterNUICallback('esx_context:closed', function(_, cb)
    if not activeMenu or (activeMenu and not activeMenu.canClose) then return cb(false) end
    cb(true)
    Closed()
end)

RegisterNUICallback('esx_context:selected', function(data, cb)
...
RegisterNUICallback('esx_context:changed', function(data, cb)
...
exports('focusPreviewContext', focusPreview)
...
if Debug then
    ...
    RegisterCommand('ctx:preview', function()
        exports.kyg_fw:PreviewContext(position, eles)
    end, false)

    RegisterCommand('ctx:open', function()
        exports.kyg_fw:OpenContext(position, eles, onSelect, onClose)
    end, false)
    ...
    RegisterCommand('ctx:form', function()
        ...
        exports.kyg_fw:OpenContext(position, formMenu, onSelect, onClose)
    end, false)
end