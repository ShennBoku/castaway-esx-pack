...
function ESX.RefreshJobs()
    ...
    for _, v in ipairs(jobs) do
        Jobs[v.name] = {
            ...
            label = v.label,
            grades = {},
            whitelisted = tonumber(v.whitelisted) == 1
        }
    end
    ...
    for _, v in ipairs(jobGrades) do
        if Jobs[v.job_name] then
            Jobs[v.job_name].grades[tostring(v.grade)] = {
                grade = v.grade,
                name = v.name,
                label = v.label,
                salary = v.salary,
    ...
    if not Jobs then
        ...
        ESX.Jobs["unemployed"] = { name = "unemployed", label = "Unemployed", whitelisted = false, grades = { ["0"] = { grade = 0, name = "unemployed", label = "Unemployed", salary = 200, skin_male = {}, skin_female = {}, is_boss = false } } }
    else
        ESX.Jobs = Jobs
    end
